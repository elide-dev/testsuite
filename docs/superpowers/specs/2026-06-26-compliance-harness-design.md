...
All three filed in `elide-dev/runtime` (related to meta #1172 and Test262 #1173):

1. **Native `print()`** (d8/Graal-style: args space-joined + newline to stdout).
   Lets us drop the `console.log` shim. *Minor.* — **elide-dev/runtime#1182**
...
3. **`$262` / Realm host hooks** (`createRealm`, `evalScript`, `detachArrayBuffer`)
   ...
   slices of Test262. *Coverage expanders, later.* — **elide-dev/runtime#1184**
