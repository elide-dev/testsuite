...
import { loadRegistry, type TargetName } from "./registry";
...
import type { ElideIdentity, Result, RunMeta, TestResult } from "./results/schema";
...
export function parseArgs(argv: string[]): CliOptions {
  ...
  const target = get("--target", "elide");
  if (target !== "elide" && target !== "bali") throw new Error("--target must be 'elide' or 'bali'");
  return {
    command,
    workload,
    target,
...
export async function main(o: CliOptions): Promise<number> {
  ...
  if (wl.target !== o.target)
    throw new Error(`workload '${wl.id}' targets ${wl.target}; run it with --target ${wl.target}`);

  const identity = await resolveTargetIdentity(o.target, o.elidePath, o.digest);
  ...
  if (o.ratchet) {
    ...
    const header =
      ...
      `# Generated for ${o.target} ${identity.semver} (sha ${identity.digest.slice(0, 12)}) on ${finishedAt}.`;
    // Merge, don't rewrite: a scoped run (--include) observes only a slice, so
    // prior entries for unobserved tests must be retained. Entries observed to
    // pass in this run are pruned (that's the ratcheting-down semantics).
    const path = ratchetPath(o.expectationsDir, wl.id);
    const merged = mergeRatchet(loadRatchet(path), new Set(tests.map((t) => t.id)), ids);
    writeRatchet(path, merged, header);
    exp.ratchet = new Set(merged);
  ...
  // Suite-specific extras (e.g. Bali's differential view) live beside the generic files.
  if (adapter.reports)
    for (const [name, content] of Object.entries(await adapter.reports(ctx, reportableResults)))
