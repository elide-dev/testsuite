...
/**
 * Whether a test file drives `node:test`. `elide run` registers such tests but refuses to start
 * them (it exits 1 naming `elide test` instead), so these files go through `elide test`. ESM
 * entries stay on `elide run`: the entry wrapper below loads its target with `require`.
 */
export function usesNodeTest(rel: string, source: string): boolean {
  return !rel.endsWith(".mjs") && NODE_TEST_IMPORT_RE.test(source);
}

/**
 * Write the `elide test` entry for a node:test file, at the overlay root, and return its path
 * relative to that root.
 *
 * Two Elide gaps shape this. `elide test` resolves the entry file's relative requires against the
 * cwd rather than the file's own directory, so `test/parallel/x.js` cannot `require('../common')`
 * as an entry. Loaded through `require` from an entry that really does live in the cwd, the test
 * is an ordinary CJS module with its own `__dirname`. And `elide test` never emits `exit`, so the
 * `process.on('exit')` checks in `common` (`mustCall` counts above all) would never run and a
 * missed call would pass. A root `after` hook emits it once the file's tests settle; a check that
 * fails calls `process.exit(1)`, which `elide test` reports as a failed file. Callbacks still
 * pending at that point are counted as missed, which fails a test Node would pass rather than the
 * reverse.
 */
export function writeNodeTestEntry(overlayPath: string, rel: string): string {
  const entry = `__elide_node_test__${rel.replaceAll("/", "__")}`;
  writeFileSync(join(overlayPath, entry), `'use strict';
// Generated by the node-api harness: runs ${rel} under \`elide test\`.
...
function resultMessage(result: ProcessRunResult, nodeTest: boolean): string | undefined {
  if (result.timedOut) return "Node API test timed out";
  if (result.exitCode === 0) return undefined;
  // `elide test` reports failures as TAP on stdout; its stderr holds only the build summary.
  const output = nodeTest ? result.stdout || result.stderr : result.stderr || result.stdout;
  return output.trim() || `node-api test exited with code ${result.exitCode}`;
}
...
async function runNodeApiTask(
  ...
  const command = nodeTest
    ...
    : ["run", ...configuredElideRunArgs(ctx), task.rel];
  ...
  return [{
    ...
    message: resultMessage(result, nodeTest),
